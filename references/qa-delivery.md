...
The checker reports package corruption, missing and broken relationships, empty slides, excessive text, common Chinese/English sample copy, off-canvas geometry, likely text overflow, obvious text-box collisions, duplicate slide text, chart cache mismatch, low image DPI, distorted image ratios, missing picture alternative text, notes, external links, media, and referenced fonts. Run `font_check.py` for installed-font comparison. Overflow and collision checks are heuristics; inspect each warning against the render.
